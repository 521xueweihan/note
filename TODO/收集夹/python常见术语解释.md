...
Iterable：可迭代的  
**这两者不是一个东西**
1. 可以通过for循环的对象，统称为可迭代对象（Iterable）。例如：list，dict，set，str，tuple，迭代器等
2. 可以被next()函数调用，并不断返回下一个值的对象称为：迭代器（Iterator）。通过`iter()`函数可以把list等可迭代的对象变成迭代器。

迭代器的好处：惰性求值——只有需要返回下一个数据的时候，它才会计算。而不是一次性就开辟所有的空间，计算所有的结果。  

详解：[廖雪峰教程][1]

## 生成器
generators：生成器  
生成器是‘高效版’的迭代器，因为生成器不是一次性把所有的内容都放到内存中，而是根据计算得出来后面的值。只需要返回一个生成器对象，然后迭代这个对象，迭代这个对象的过程就是调用她的next方法。  
用文字来描述，不是很好理解，下面我就用代码来说明，生成器有多么可爱：
```python
# 假设一个需求：生成一个‘0到1111111111111111110’的可迭代对象
num_list = range(1111111111111111111) # 报错：OverflowError: range() result has too many items 太大了，生成不了

# 那怎么办呢？用生成器试试！
def generator_func(x, start=0):
	while(start<x):
		yield start
		start += 1

num_list2 = generator_func(1111111111111111111)  # 现在num_list2就是生成器对象了，可以调用next方法，进行迭代了。

# 传说中的xrange
num_list3 = xrange(1111111111111111111)  # 返回的是xrange对象，可以像数组一样使用，不信快去试试看！

# 快看‘哪吒’(魔法)
num_list4 = (i for i in xrange(1111111111111111111)) # 可以优雅的返回一个符合要求的生成器。
```
上面例子：`generator_fun`占用内存极小，每次返回的i都是方法，值是通过调用next计算值。而`range`需要开辟[1,2,3,4,5...1111111111111111110]的list，每次的值都是取出来的。
...
## 什么是哈希表和哈希算法
哈希算法并不是一个特定的算法而是一类算法的统称。哈希算法也叫散列算法，一般来说满足这样的关系：f(data)=key，输入任意长度的data数据，经过哈希算法处理后输出一个定长的数据key。同时这个过程是不可逆的，__无法由key逆推出data__。
...
[1]: http://www.liaoxuefeng.com/wiki/0014316089557264a6b348958f449949df42a6d3a2e542c000/00143178254193589df9c612d2449618ea460e7a672a366000  "可迭代的和迭代器"
